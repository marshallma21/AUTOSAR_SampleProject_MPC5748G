...
)
{
    ...
    if ((((uint32)(pChannelGroup)) >= ((uint32)Dio_ConfigPC.pChannelGroupList))                                  &&  \
        (((uint32)(pChannelGroup)) < (uint32)(Dio_ConfigPC.pChannelGroupList + Dio_ConfigPC.u8NumChannelGroups)) &&  \
        ...
        ((uint8)(pChannelGroup->offset) <= DIO_MAX_VALID_OFFSET_U8)
       )
    {
        Valid = (Std_ReturnType)E_OK;
    }
