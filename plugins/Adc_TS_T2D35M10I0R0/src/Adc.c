...
*/
VAR(Adc_DualClockModeType, AUTOMATIC) Adc_ClockMode = ADC_NORMAL;
...
)
{
...
#endif /* ADC_ENABLE_QUEUING == STD_OFF  */
    {
        ...
        if (((0U != NoGroupsInHwInjectedQueue) || (0U != NoGroupsInHwNormalQueue)) && \
            ((Adc_aUnitStatus[Unit].HwQueueGroupType != Adc_pCfgPtr->pGroups[Group].IsMHTGroup) || \
             ((Adc_MhtGroupType)ADC_REGULAR_GROUP_TYPE == Adc_aUnitStatus[Unit].HwQueueGroupType)
            )
           )
        {
...
LOCAL_INLINE FUNC(void, ADC_CODE) Adc_InitUnitStatus(void)
{
    ...
    for(Unit = 0U; Unit < (Adc_HwUnitType)ADC_MAX_HW_UNITS; Unit++)
    {
        ...
        Adc_aUnitStatus[Unit].HwQueueGroupType = ADC_REGULAR_GROUP_TYPE;
    #endif /* (ADC_MULTIPLE_HARDWARE_TRIGGERS == STD_ON) */
#endif /* (ADC_HW_TRIGGER_API == STD_ON) */
    }
#if (ADC_POWER_STATE_SUPPORTED == STD_ON) || defined(__DOXYGEN__)
...
)
{
    ...
    /** @violates @ref Adc_c_REF_4 Array indexing shall be the only allowed form of pointer arithmetic */
    eChRange = Adc_ChannelsLimitCheckingCfg[HwIndex][Index].eChannelRange;
    /** @violates @ref Adc_c_REF_4 Array indexing shall be the only allowed form of pointer arithmetic */
    LowLimit = (Adc_ValueGroupType)Adc_ChannelsLimitCheckingCfg[HwIndex][Index].ChannelLowLimit;
    /** @violates @ref Adc_c_REF_4 Array indexing shall be the only allowed form of pointer arithmetic */
    HighLimit = (Adc_ValueGroupType)Adc_ChannelsLimitCheckingCfg[HwIndex][Index].ChannelHighLimit;
        ...
        if((Std_ReturnType)E_OK == Adc_ValidatePowerStatePrepare(ADC_SETPOWERSTATE_ID, Result))
        {
            if((Std_ReturnType)E_OK == Adc_ValidatePowerStateSupport(ADC_SETPOWERSTATE_ID, Adc_eTargetState, Result))
            {
                if((Std_ReturnType)E_OK == Adc_ValidateIdleState(ADC_SETPOWERSTATE_ID, Result))
                {
                    ...
                    {
                        /* The target state is the same as the current one, return a "Sequence Error" status */
...
)
{
    ...
    {   
        ...
        {
            if(Adc_eCurrentState != PowerState)
            {
                /* Everything is ok */
            ...
            {
                /* Current state is the same prepared state */
                *Result = ADC_SEQUENCE_ERROR;
            }
        }       
    }
