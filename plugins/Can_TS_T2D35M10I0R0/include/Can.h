...
typedef struct
{
    CONST(uint64, CAN_CONST) CanIcomSignalMask;
    CONST(Can_IcomSignalOperationType, CAN_CONST) CanIcomSignalOperation;
    CONST(uint64, CAN_CONST) CanIcomSignalValue;
...
typedef struct
...
#else        
        CONST(Can_HwWidth, CAN_CONST) u8CanBtr0Register; /**< @brief Content of the Control Register (CTRL) fields: PRESDIV, RJW, PSEG1, PSEG2, CLKSRC, LPB, SMP, BOFF_REC, LOM, PROPSEG. */
  ...
  #ifdef CAN_MSCAN_SPECIFIC
        CONST(Can_HwWidth, CAN_CONST) u32ControlRegisterAlternate; /**< @brief Content of the Control Register (CTRL) fields: PRESDIV, RJW, PSEG1, PSEG2, CLKSRC, LPB, SMP, BOFF_REC, LOM, PROPSEG. */
  #else
...
typedef struct
  ...
  #ifdef CAN_MSCAN_SPECIFIC
        CONST(Can_HwWidth, CAN_CONST) u32ControlRegisterAlternate; /**< @brief Content of the Control Register (CTRL) fields: PRESDIV, RJW, PSEG1, PSEG2, CLKSRC, LPB, SMP, BOFF_REC, LOM, PROPSEG. */
        CONST(Can_HwWidth, CAN_CONST) u8CanBtr1RegisterAlternate;
  #else
        CONST(uint32, CAN_CONST) u32ControlRegisterAlternate; /**< @brief Content of the Control Register (CTRL) fields: PRESDIV, RJW, PSEG1, PSEG2, CLKSRC, LPB, SMP, BOFF_REC, LOM, PROPSEG. */
  #endif
#endif /* (CAN_DUAL_CLOCK_MODE == STD_ON) */
...
typedef struct
    {
...
#ifdef CAN_MSCAN_SPECIFIC        
        VAR(Can_FilterConfigPtrType, CAN_VAR) pConfigFilterMask;
#endif
#ifndef CAN_MCAN_SPECIFIC
        ...
        CONST(uint8, CAN_CONST) u8ControllerIdMapping[CAN_MAX_OBJECT_ID];

        /*Can Object Type mapping*/
        CONST(Can_ObjType, CAN_CONST) eObjectTypeMapping[CAN_MAX_OBJECT_ID];
