...
*/
static VAR(Fee_BlockConfigType, FEE_VAR) Fee_ForeignBlockConfig[FEE_MAX_NR_OF_BLOCKS - FEE_CRT_CFG_NR_OF_BLOCKS]={{0U}};
#endif
...
static FUNC( uint16, FEE_CODE ) Fee_GetForeignBlockIndex( CONST( uint16, AUTOMATIC ) uBlockNumber )
{
    ...
    if (Fee_uForeignBlocksNumber >= 1UL)
    {
        ...
        {
            if(uBlockNumber == Fee_ForeignBlockConfig[uForeignBlockIt].blockNumber)
            {
...
LOCAL_INLINE FUNC( Fee_BlockAssignmentType, FEE_CODE ) Fee_GetBlockAssignment(CONST( uint16, AUTOMATIC ) uBlockRuntimeInfoIndex)
{
    ...
    else
    {
        ...
        uBlockAssignment = Fee_ForeignBlockConfig[uBlockRuntimeInfoIndex-FEE_CRT_CFG_NR_OF_BLOCKS].blockAssignment;
    }
    #else        
...
LOCAL_INLINE FUNC( uint8, FEE_CODE ) Fee_GetBlockClusterGrp(CONST( uint16, AUTOMATIC ) uBlockRuntimeInfoIndex)
{
    ...
    {
        /* this means block is foreign, so the config is part of Fee_ForeignBlockConfig*/
        uBlockClusterGrp = Fee_ForeignBlockConfig[uBlockRuntimeInfoIndex-FEE_CRT_CFG_NR_OF_BLOCKS].clrGrp;
    }
    #else        
    /* the config is part of Fee_BlockConfig*/
        uBlockClusterGrp = Fee_BlockConfig[uBlockRuntimeInfoIndex].clrGrp;
...
LOCAL_INLINE FUNC( uint16, FEE_CODE ) Fee_GetBlockSize(CONST( uint16, AUTOMATIC ) uBlockRuntimeInfoIndex)
{
    ...
    {
        /* this means block is foreign, so the config is part of Fee_ForeignBlockConfig*/
        uBlockSize = Fee_ForeignBlockConfig[uBlockRuntimeInfoIndex-FEE_CRT_CFG_NR_OF_BLOCKS].blockSize;
    }
    #else        
    /* the config is part of Fee_BlockConfig*/
        uBlockSize = Fee_BlockConfig[uBlockRuntimeInfoIndex].blockSize;
...
LOCAL_INLINE FUNC( uint16, FEE_CODE ) Fee_GetBlockNumber(CONST( uint16, AUTOMATIC ) uBlockRuntimeInfoIndex)
{
    ...
    {
        /* this means block is foreign, so the config is part of Fee_ForeignBlockConfig*/
        uBlockNumber = Fee_ForeignBlockConfig[uBlockRuntimeInfoIndex-FEE_CRT_CFG_NR_OF_BLOCKS].blockNumber;
    }
    #else        
    /* the config is part of Fee_BlockConfig*/
        uBlockNumber = Fee_BlockConfig[uBlockRuntimeInfoIndex].blockNumber;
...
LOCAL_INLINE FUNC( boolean, FEE_CODE ) Fee_GetBlockImmediate(CONST( uint16, AUTOMATIC ) uBlockRuntimeInfoIndex)
{
    ...
    {
        /* this means block is foreign, so the config is part of Fee_ForeignBlockConfig*/
        bImmediate = Fee_ForeignBlockConfig[uBlockRuntimeInfoIndex-FEE_CRT_CFG_NR_OF_BLOCKS].immediateData;
    }
    #else        
        /* the config is part of Fee_BlockConfig*/
        bImmediate = Fee_BlockConfig[uBlockRuntimeInfoIndex].immediateData;
                ...
                {       
                    ...
                    if( 0xFFFFU == uForeignBlockIndex )
                    {
                        ...
                        if(Fee_uForeignBlocksNumber < (FEE_MAX_NR_OF_BLOCKS - FEE_CRT_CFG_NR_OF_BLOCKS))
                        {
                            ...
                            Fee_ForeignBlockConfig[uForeignBlockIndex].blockNumber = uBlockNumber;
                            Fee_ForeignBlockConfig[uForeignBlockIndex].blockSize = uBlockSize;
                            Fee_ForeignBlockConfig[uForeignBlockIndex].clrGrp = Fee_uJobIntClrGrpIt;
                            Fee_ForeignBlockConfig[uForeignBlockIndex].immediateData = bImmediateData;
                            Fee_ForeignBlockConfig[uForeignBlockIndex].blockAssignment = (Fee_BlockAssignmentType)uBlockAssignment; 
...
{
    return (0x00000001UL << (uInVal & 0x1FUL));
}
...
FUNC( void, FEE_CODE ) Fee_Init(P2CONST( Fee_ConfigType, AUTOMATIC, FEE_APPL_CONST ) ConfigPtr)
{
   VAR( uint32, AUTOMATIC ) uInvalIndex = 0U;
...
#endif    /* FEE_DEV_ERROR_DETECT == STD_ON */
    {
        ...
        {
            Fee_ForeignBlockConfig[uInvalIndex].blockNumber = 0U;
            Fee_ForeignBlockConfig[uInvalIndex].blockSize = 0U;
            Fee_ForeignBlockConfig[uInvalIndex].clrGrp = 0U;
            Fee_ForeignBlockConfig[uInvalIndex].immediateData = (boolean)FALSE;
            Fee_ForeignBlockConfig[uInvalIndex].blockAssignment = FEE_PROJECT_RESERVED;
        }
#endif
    ...
    )
{
    ...
    */
    ...
    else
    {
        ...
        ClrGrpRTInfo.clusterTotalSpace =
            /* @violates @ref Fee_c_REF_6 Array indexing shall be the only
               allowed form of pointer arithmetic. */
        ...
        ClrGrpRTInfo.blockHeaderOverhead  = (uint8)FEE_CLUSTER_OVERHEAD;

        ClrGrpRTInfo.virtualPageSize = (uint8)FEE_VIRTUAL_PAGE_SIZE;

        ClrGrpRTInfo.numberOfSwap = Fee_aClrGrpInfo[ uClrGrpIndex ].uActClrID - 1U;
    }
