...
* @retval        ETH_E_NO_ACCESS Inaccessible to tranceiver.
                                                ...
                                                )
{
    ...
    if(CtrlIdx >= ETH_MAXCTRLS_SUPPORTED)
    {
        (void)Det_ReportError ( \
            (VAR(uint8, AUTOMATIC)) ETH_MODULE_ID, \
            ETH_DRIVER_INSTANCE, (VAR(uint8, AUTOMATIC)) \
            ETH_SID_GETDROPCOUNT, ETH_E_INV_CTRL_IDX \
                              );
    }
    else
    {
        ...
        else
        {
            if((NULL_PTR == DropCountPtr)||(NULL_PTR == CountValuesPtr))
            {
                (void)Det_ReportError ( \
                    (VAR(uint8, AUTOMATIC)) ETH_MODULE_ID, \
                    ETH_DRIVER_INSTANCE, (VAR(uint8, AUTOMATIC)) \
                    ETH_SID_GETDROPCOUNT, ETH_E_PARAM_POINTER \
                                      );
            }
            else
            {
                                                    ...
                                                    )
{
    ...
    if(CtrlIdx >= ETH_MAXCTRLS_SUPPORTED)
    {
        (void)Det_ReportError ( \
            (VAR(uint8, AUTOMATIC)) ETH_MODULE_ID, \
            ETH_DRIVER_INSTANCE, (VAR(uint8, AUTOMATIC)) \
            ETH_SID_GETETHERSTATS, ETH_E_INV_CTRL_IDX \
                              );
    }
    else
    {
        ...
        else
        {
            if(NULL_PTR == etherStats)
            {
                (void)Det_ReportError ( \
                    (VAR(uint8, AUTOMATIC)) ETH_MODULE_ID, \
                    ETH_DRIVER_INSTANCE, (VAR(uint8, AUTOMATIC)) \
                    ETH_SID_GETETHERSTATS, ETH_E_PARAM_POINTER \
                                      );
            }
            else
            {
